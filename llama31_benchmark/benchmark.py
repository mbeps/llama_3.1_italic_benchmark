...
class Llama31Benchmark:
    ...
    def _merge_lora_adapters(self) -> str:
        ...
        try:
            ...
            try:
                ...
                if not any(
                    f.startswith("tokenizer") for f in os.listdir(tokenizer_path)
                ):
    ...
    def load_dataset(self) -> List[Dict[str, Any]]:
        ...
        if (
            ...
            and len(full_dataset) > self.config.max_eval_samples
        ):
            if self.config.use_uniform_subset:
                dataset = create_uniform_subset(
                    full_dataset, self.config.max_eval_samples
                )
            else:
    ...
    def load_model(self):
        ...
        if self.config.lora_adapter_path:
            ...
            print(f"✓ Base model: {self.config.model_name}")
        print(
            f"✓ Temperature: {self.config.temperature} (deterministic)"
            if self.config.temperature == 0.0
            else f"✓ Temperature: {self.config.temperature}"
        )
    ...
    def generate_batch_responses(
        ...
        outputs = self.model.chat(
            batch_messages, sampling_params=self.sampling_params, use_tqdm=False
        )
    ...
    def run_evaluation(
        ...
        for batch_start in pbar:
            ...
            try:
                ...
                # Process results
                for i, (question_data, response, correct_answer) in enumerate(
                    zip(batch_data, batch_responses, batch_correct_answers)
                ):
                    ...
                    result = {
                        ...
                        "category": category,
                        "macro_category": question_data.get(
                ...
                for i, question_data in enumerate(batch_data):
                    total += 1
                    category_stats[question_data.get("category", "unknown")][
                        "total"
                    ] += 1
    ...
    def save_results(
        ...
        if self.config.save_summary:
            ...
            overall_accuracy = (
                sum(result["is_correct"] for result in results) / len(results) * 100
            )
            ...
            if self.config.lora_adapter_path:
                adapter_name = os.path.basename(
                    self.config.lora_adapter_path.rstrip("/")
                )
                model_info.update(
                    {
                        "fine_tuning": "LoRA",
    ...
    def analyze_results_by_category(
        ...
        for category, stats in sorted(category_stats.items()):
            ...
            print(
                f"{category:25s} {accuracy:11.2f}% {stats['correct']:>6d}/{stats['total']:<6d}"
            )

        print("-" * 60)
        print(
            f"{'TOTAL':25s} {overall_accuracy:11.2f}% {total_correct:>6d}/{total_questions:<6d}"
        )
    ...
    def test_inference(self):
        ...
        test_messages, test_answer = configure_payload_zero_shot(
            self.dataset[0], self.config.system_message, self.config.query_template
        )
