...
class Llama31BenchmarkConfig:
    """
    Configuration for Llama 3.1 benchmarking with vLLM and LoRA support.
    """

    model_name: Optional[str] = None
    min_vram_gb: float = 16.0
    lora_adapter_path: Optional[str] = None
    merge_adapter_in_memory: bool = True
...
""".strip()
    ...
    def __post_init__(self):
        """
        Perform post-initialization setup for the configuration.
        """
        ...
        if self.hf_token:
            ...
            print(
                "No HF token found in environment - proceeding without authentication"
            )
    ...
    def _validate_lora_config(self):
        """
        Validate the LoRA adapter configuration.
        """
        if not os.path.exists(self.lora_adapter_path):
            raise FileNotFoundError(
                f"LoRA adapter path not found: {self.lora_adapter_path}"
            )
    ...
    def _resolve_model_name_from_adapter(self):
        """
        Set base model name from adapter config.
        """
        import json

        adapter_config_path = os.path.join(
            self.lora_adapter_path, "adapter_config.json"
        )
        ...
        try:
            ...
            if self.model_name and self.model_name != base_model_name:
                print(
                    f"⚠️  Warning: Provided model_name '{self.model_name}' differs from adapter's base model '{base_model_name}'"
                )
    ...
    def _auto_detect_batch_size(self) -> int:
        ...
        if total_memory < self.min_vram_gb:
            raise ValueError(
                f"Insufficient VRAM: {total_memory:.1f}GB available, {self.min_vram_gb:.1f}GB required"
            )
        ...
        if self.min_vram_gb >= 140:
            batch_size = max(
                1, min(4, int(available_for_batching * memory_multiplier // 2))
            )
        elif self.min_vram_gb >= 32:
            batch_size = max(
                2, min(8, int(available_for_batching * memory_multiplier // 1))
            )
        else:
            batch_size = max(
                4, min(16, int(available_for_batching * memory_multiplier // 0.5))
            )
